...
class YoutubeSlideGenerator:
    ...
    def generate_image(self, scene, output_path: str):
        ...
        try:
            ...
            main_title_font = ImageFont.truetype(font_path, 130) if font_path else ImageFont.load_default()
            subtitle_font = ImageFont.truetype(font_path, 65) if font_path else ImageFont.load_default()
            body_font = ImageFont.truetype(font_path, 50) if font_path else ImageFont.load_default()
        except OSError:
        ...
        if scene.is_title:
            ...
            if data.get("type") == "chart":
                ...
                if num_steps > 0:
                    ...
                    step_font_path = branding().font_path
                    if not step_font_path:
                        import sys
                        step_font_path = "/System/Library/Fonts/Supplemental/Arial.ttf" if sys.platform == "darwin" else "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"
                    try:
                        step_font = ImageFont.truetype(step_font_path, 40)
                    except OSError:
