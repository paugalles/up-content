...
SCHEMA = {
    ...
    "properties": {
        ...
        "slides": {"type": "array", "minItems": 3, "maxItems": 5, "items": {
...
def _dry_content(article: Article) -> dict:
    ...
    while len(slides) < 5:
        slides.append({"label": str(len(slides) + 1), "title": brand.site, "body": f"{cta} {disclaimer}"})
    return {"title": article.title[:100], "caption": f"{article.title}\n\n{cta} {disclaimer}\n{article.canonical_url}", "hashtags": ["#information", "#news"], "slides": slides, "narration": " ".join(s["body"] for s in slides)}
...
Make 3-5 logically ordered slides, useful narration under 600 words, and a compelling platform-neutral caption. Make the content specific, direct to the point, and shorter.
