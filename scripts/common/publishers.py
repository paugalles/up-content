...
def instagram(assets: list[Path], caption: str, http=Http(attempts=6)):
    env = require("INSTAGRAM_ACCOUNT_ID", "FACEBOOK_PAGE_ACCESS_TOKEN")
    ...
    for path in assets:
        ...
        if len(assets) > 1:
            child = http.json("POST", f"{GRAPH}/{env['INSTAGRAM_ACCOUNT_ID']}/media", data={"image_url": url, "is_carousel_item": "true", "access_token": env["FACEBOOK_PAGE_ACCESS_TOKEN"]})["id"]
            poll(lambda: http.json("GET", f"{GRAPH}/{child}", params={"fields": "status_code", "access_token": env["FACEBOOK_PAGE_ACCESS_TOKEN"]}), lambda x: x.get("status_code") == "FINISHED", "Instagram child", failed=lambda x: x.get("status_code") in {"ERROR", "EXPIRED"})
            children.append(child)
        else:
            # Single image post
            parent = http.json("POST", f"{GRAPH}/{env['INSTAGRAM_ACCOUNT_ID']}/media", data={"image_url": url, "caption": caption, "access_token": env["FACEBOOK_PAGE_ACCESS_TOKEN"]})["id"]
            poll(lambda: http.json("GET", f"{GRAPH}/{parent}", params={"fields": "status_code", "access_token": env["FACEBOOK_PAGE_ACCESS_TOKEN"]}), lambda x: x.get("status_code") == "FINISHED", "Instagram media", failed=lambda x: x.get("status_code") in {"ERROR", "EXPIRED"})
            
            import time
            time.sleep(20)
            return http.json("POST", f"{GRAPH}/{env['INSTAGRAM_ACCOUNT_ID']}/media_publish", data={"creation_id": parent, "access_token": env["FACEBOOK_PAGE_ACCESS_TOKEN"]})["id"]
            
    # Carousel post (requires 2 or more images)
    parent = http.json("POST", f"{GRAPH}/{env['INSTAGRAM_ACCOUNT_ID']}/media", data={"media_type": "CAROUSEL", "children": ",".join(children), "caption": caption, "access_token": env["FACEBOOK_PAGE_ACCESS_TOKEN"]})["id"]
    poll(lambda: http.json("GET", f"{GRAPH}/{parent}", params={"fields": "status_code", "access_token": env["FACEBOOK_PAGE_ACCESS_TOKEN"]}), lambda x: x.get("status_code") == "FINISHED", "Instagram carousel", failed=lambda x: x.get("status_code") in {"ERROR", "EXPIRED"})
