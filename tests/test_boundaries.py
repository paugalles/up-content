...
class BoundaryTests(unittest.TestCase):
    def test_openai_structured_json_boundary(self):
        ...
        with patch.dict("os.environ", {"OPENAI_API_KEY": "test"}, clear=True), patch("openai.OpenAI", return_value=client):
    ...
    def test_facebook_uses_graph_photo_api(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            http = Mock(); http.json.return_value = {"id": "post-1"}
            env = {"FACEBOOK_PAGE_ID": "page", "FACEBOOK_PAGE_ACCESS_TOKEN": "token"}
            with patch.dict("os.environ", env, clear=True):
                self.assertEqual(publishers.facebook(image, "copy", http), "post-1")
            self.assertIn("/page/photos", http.json.call_args.args[1])
